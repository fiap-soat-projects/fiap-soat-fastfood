...
{
    private readonly IMenuItemGateway _menuItemGateway;

    public MenuItemUseCase(IMenuItemGateway menuItemGateway)
    {
        _menuItemGateway = menuItemGateway;
    }

    public async Task<MenuItem> CreateAsync(MenuItem item, CancellationToken cancellationToken)
    {
        item = await _menuItemGateway.CreateAsync(item, cancellationToken);
    ...
    public async Task<MenuItem> GetByIdAsync(string id, CancellationToken cancellationToken)
    {
        ...
        var item = await _menuItemGateway.GetByIdAsync(id, cancellationToken);
    ...
    {
        var items = await _menuItemGateway.GetAllAsync(filter, cancellationToken);
    ...
    public async Task<MenuItem> UpdateAsync(MenuItem item, CancellationToken cancellationToken)
    {
        ...
        return await _menuItemGateway.UpdateAsync(item.Id, item, cancellationToken);
    }
